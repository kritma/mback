...
export async function initDatabase(db: Database) {
    ...
    await db.run("create table if not exists songs(id integer primary key autoincrement, user_id integer not null, name nvarchar(150) not null, url varchar(200) not null)")
    await db.run("create table if not exists favorite_songs(song_id integer not null, user_id integer not null)")
}