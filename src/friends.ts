...
router.get('/api/friends', auth, async (req, res) => {
    ...
    const users = await db.all<string[]>("select u.id, u.name, u.image_url from friends f where iif(f.user1_id = ?, f.user1_id, f.user2_id) = u.id and (f.user1_id = ? or f.user2_id = ?)", userId, userId, userId)
    res.json(users)
})
